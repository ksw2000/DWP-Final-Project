...
function render_file_list($file){
    ...
    foreach($file_list as $v){
        ...
        $ret .= '<div class="file-list-item" data-file="'.$v['FILE_TYPE'].'/'.$v['SERVER_NAME'].'">';
        ...
        $ret .= '<center><button class="blue delete-button" onclick="delete_file(\''.$v['SERVER_NAME'].'\')">刪除</button></center>';
        $ret .= '<center><button class="blue confirm-delete-button" onclick="real_delete_file(\''.$v['SERVER_NAME'].'\')">確認刪除</button></center>';
...
function redner_user_list($user){
    ...
    foreach($list as $v) {
        $online_time = '';
        if($v['ONLINE'] != 0){
            $online_time = '<p>'.text_r(render_time($v['ONLINE']).'上線', render_time($v['ONLINE']).'上线', 'Online '.render_time($v['ONLINE'])).'</p>';
        }
        ...
        <div class="col setting-user-area" data-user-id="'.$v['ID'].'" style="display:none;">'.$online_time.$manage_board.'
            <p><a target="_blank" href="/user/'.$v['ID'].'" id="setting-user-personal-page-link">'.text_r('前往個人頁', '前往个人页','Profile').'</a></p>
            <select class="normal" id="choose-permission">
                <option value="0" '.$select_0.'>'.text_r('一般會員', '一般会员','Member').'</option>
                <option value="1" '.$select_1.'>'.text_r('管理員', '管理员','Administrator').'</option>
            </select>
            <center style="margin: 15px 0px;"><button class="blue" onclick="update_user_permission(\''.$v['ID'].'\')">'.text_r('權限變更', '权限变更','Authority Changed').'</button></center>
        </div>
        ';
    }
